...
The mechanisms by which we enforce access control checks vary, and the `foam.core.auth` package provides a number of different ways to enforce access control, which are documented in more detail below.
...
Services are represented by the `foam.core.boot.CSpec` model. `CSpec`s have two properties related to authorization:
