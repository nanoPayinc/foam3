  ...
  methods: [
    {
      ...
      code: async function(x, permissionId) {
        ...
        var arraySink = await this.permissions.junctionDAO
          .where(this.EQ(foam.core.auth.GroupPermissionJunction.SOURCE_ID, this.id))
          .select();
        var junctions = arraySink != null && Array.isArray(arraySink.array)
          ? arraySink.array
          : [];
        return junctions.some((j) => foam.core.auth.Permission.create({ id: j.targetId }).implies(permissionId));
      }
    },
    {
    ...
    },
    {
      ...
      args: [
        ...
        { name: 'group', type: 'foam.core.auth.Group' }
      ],
      javaCode: `
        do {
    ...
    },
    {
      ...
      for ( foam.net.CIDR cidr : cidrs ) {
        try {
          ...
          ((foam.core.logger.Logger) x.get("logger")).warning(this.getClass().getSimpleName(), "validateCidrWhiteList", remoteIp, e.getMessage());
        }
      }
      ((foam.core.logger.Logger) x.get("logger")).debug(this.getClass().getSimpleName(), "validateCidrWhiteList", "Restricted IP address not allowed", remoteIp, getId());
      throw new foam.lang.ValidationException("Restricted IP");
      `
    }
  ]
});
