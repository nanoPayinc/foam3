...
public class PreventPrivilegeEscalationTest
  ...
  public void runTest(X x) {
    ...
    testGroup = null;
    testUser = null;
    logger_ = new foam.core.logger.PrefixLogger(new Object[] {"PreventPriviledgeEscalation"}, (Logger) x.get("logger"));
    spid_ = generateId();
  ...
  X generateTestUser(X x, List<String> permissionIds) {
    ...
    testUser = new User.Builder(x)
      ...
      .setGroup(groupId)
      .setSpid(spid_)
      .setLifecycleState(foam.core.auth.LifecycleState.ACTIVE)
      .build();
  ...
  void createUserInAdminGroupThrows(X x) {
    ...
    try {
      ...
      test(null == localUserDAO.find(foam.mlang.MLang.EQ(foam.core.auth.User.EMAIL, "ppet+admin@foamdev.com")), "User was not created with that email address.");
      userDAO.remove(u);
