...
import foam.core.dig.exception.DigErrorMessage;
import foam.core.dig.exception.GeneralException;
  ...
  implements SendErrorHandler
{
  ...
  public void execute(X x) {
    ...
    try {
      ...
      CSpec nspec = (CSpec) cSpecDAO.find(serviceName);
      ...
      for (var method : method_) {
        if (method.getName().equals(methodName)) { //found picked Method
          ...
          for (int j = 0; j < pArray.length; j++) { // checking the method's each parameter
            ...
            if (typeName.equals("foam.lang.X")) {
              arglist[j] = x;
  ...
  protected Object getFieldInfo(X x, String className, HttpParameters p) {  // For Obj Parameters
    ...
    try {
      ...
      List axioms = ((foam.lang.FObject) clsObj).getClassInfo().getAxiomsByClass(PropertyInfo.class);
