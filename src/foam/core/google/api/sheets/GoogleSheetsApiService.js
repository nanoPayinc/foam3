...
foam.CLASS({
  package: 'foam.core.google.api.sheets',
  ...
  javaImports: [
    ...
    'foam.core.google.api.drive.GoogleDriveService',
  ...
  methods: [
      ...
      javaCode: `
        ...
        for ( int i = 0 ; i < metadata.length ; i++ ) {
          ...
          if ( metadata[i].getCellType().equals("CURRENCY") ) {
            for ( int j = 0 ; j < metadata[i].getPerValuePattercSpecificValues().length ; j++ ) {
              if ( metadata[i].getPerValuePattercSpecificValues()[j] == null || metadata[i].getPerValuePattercSpecificValues()[j].equals("") || metadata[i].getPerValuePattercSpecificValues()[j].equals(DEFAULT_CURRENCY) )
                continue;
              requests.add(new Request().setRepeatCell(
                new RepeatCellRequest()
                  .setCell(new CellData().setUserEnteredFormat(new CellFormat().setNumberFormat(new NumberFormat().setType(metadata[i].getCellType()).setPattern("\\"$\\"#0.00\\" " + metadata[i].getPerValuePattercSpecificValues()[j] + "\\""))))
    ...
    },
    {
      ...
      javaCode: `
        ...
        Projection p = ( new foam.core.column.ExpressionForArrayOfNestedPropertiesBuilder() ).buildProjectionForPropertyNamesArray(x, of, propNamesArr);
        dao.select(p);
        ...
        for ( int i = 0 ; i < indexesOfUnitValuePropertyName.size() ; i++ ) {
          ...
          String[] pattercSpecificValues = new String[values.size()];
          for ( int j = 0 ; j < values.size() ; j++ ) {
            pattercSpecificValues[j] = (String) values.get(j)[indexForMetadataPropertyUnitPropertyName];
          }
          metadata[i].setPerValuePattercSpecificValues(pattercSpecificValues);
        }
